...
export default class Drawer extends Component {
  ...
  processShouldSet = (e, gestureState) => {
    ...
    let whichPan = this.testBtnPanResponderMask(e, gestureState);

    if(whichPan !== null && whichPan !== this.props.side){
        inMask = true
    }
  ...
  testBtnPanResponderMask = (e, gestureState) => {
      if(!this._open){
        let x0 = e.nativeEvent.pageX
        let y0 = e.nativeEvent.pageY
        if(x0 < this._xMax && y0 < this._yMax) {
          if(!this._whichPann){
            this._whichPann = 'right'
          }else if(this._whichPann.toString() === 'left'){
            this._whichPann = null;
            return 'left'
          }
          return 'right';
        }
        if(x0 > (this.state.viewport.width - this._xMax) && y0 < this._yMax){
          if(!this._whichPann){
            this._whichPann = 'left'
          }else if(this._whichPann.toString() === 'right'){
            this._whichPann = null;
            return 'right'
          }
          return 'left';
