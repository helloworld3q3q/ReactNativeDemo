...
class SwipeCards extends Component {
  ...
  _goToNextCard(press) {
    ...
    if(press === undefined || press === null){
      this.state.direction ? newIdx = currentCardIdx + 1 : prev < 0 ? newIdx = len : newIdx = prev;
    }else{
      press === 'right' ? newIdx = currentCardIdx + 1 : prev < 0 ? newIdx = len : newIdx = prev;
    }
  ...
  _cardList() {
    ...
    let list = [];
   // let rank = this.state.card.rank; this.props.cards.indexOf(this.state.card);
    let rank = this.props.cards.indexOf(this.state.card) + 1;
    ...
    if(this.state.direction){
      for(let i = 0; i < 3; i++){
        ...
        card.key = i;
        cardsView.unshift(this.renderCard(card, false));
      }
      ...
      for(let i = 4; i > 0; i--){
        ...
        card.CardTop = (i + 1)^3 * (i + 2) ;
        card.key = i;
        cardsView.push(this.renderCard(card, false));
      }
    }
    return cardsView;
  }
  ...
  pressNext = (str) => {
    ...
    Animated.spring(this.state.pan, {
      toValue: valueXY,
      bounciness: 10,
    }).start(this._resetState.bind(this, str));
  ...
  renderCard(cardData, lastBoo) {
    if(lastBoo) {
      let id = cardData.subject ? cardData.subject.id : cardData.id;
      AsyncStorage.setItem('panId', id);
    }
            ...
            <View style={this.props.containerStyle}>
                ...
                { this.state.card
                    ...
                    <Animated.View style={[this.props.cardStyle, animatedCardstyles]} {...this._panResponder.panHandlers}>
                        {this.renderCard(this.state.card, true)}
                    </Animated.View>
                )
                ...
                { this.props.renderNope
                  ? this.props.renderNope(pan, this.pressNext)
                  : (
                      this.props.showNope
                      ? (
                ...
                { this.props.renderYup
                  ? this.props.renderYup(pan, this.pressNext)
                  : (
                      this.props.showYup
                      ? (
